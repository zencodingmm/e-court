...
import { EVoteAttachment, EVote } from '../../../../../../types/ecourt';
...
const Attachment = () => {
    ...
    const [attachment, setAttachment] = useState<EVoteAttachment[] | undefined>();
    ...
    const onSubmitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            files?.map(async (file: File) => {
                const response = await axiosInstance.post('/api/e_vote_attachment', { file, e_vote_id: evote?.e_vote_id }, { headers: { 'Content-Type': 'multipart/form-data' } });
    ...
    const onDeleteHandler = async (id: number | undefined) => {
        try {
            const response = await axiosInstance.delete(`/api/e_vote_attachment/${id}`);
            const { message } = response.data;
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const response = await axiosInstance.get(`/api/e_vote_attachment?page=${first}&page_size=${rows}&id=${id}`);
    ...
    const fileTemplate = (data: EVoteAttachment) => {
        console.log(data);
        ...
        if (data.file_type.includes(FileType.image)) {
            return (
                <Image
                    ...
                    src={`${process.env.NEXT_PUBLIC_EVOTES_URL}/${data.file_name}`}
                    alt={data.file_name}
                    width={1920}
                    height={1080}
                    onClick={() => {
                        setDialogVisible(true);
                        setUrl(`${process.env.NEXT_PUBLIC_EVOTES_URL}/${data.file_name}`);
        ...
        <div>
            ...
            <div className='col flex align-items-center gap-4'>
                <Button
                    ...
                    onClick={() => router.back()}
                />
                <h1 className='text-xl mb-5'>E-Vote Attachment</h1>
            </div>
                ...
                {!isLoading && (
                    ...
                    >
                        ...
                        <Column
                            field='created_at'
                            header='ရက်စွဲ'
                            body={(data: EVoteAttachment) => data.created_at && new Date(data.created_at).toLocaleDateString()}
                        />
                        <Column
                            align='center'
                            body={(data: EVoteAttachment) => (
                                <Button
                                    label='Download'
                                    icon='pi pi-download'
                                    onClick={() => downloadHandler(`${process.env.NEXT_PUBLIC_EVOTES_URL}/${data.file_name}`, data.file_name.split('/')[1])}
                                />
                            )}
                        />
                        <Column
